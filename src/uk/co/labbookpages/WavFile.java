...
public class WavFile {
	...
	public static WavFile openWavFile(InputStream iStream, long fileLength) throws IOException, WavFileException {
		...
		if (fileLength != chunkSize + 8) {
			throw new WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + fileLength + ")");
		}
	...
	public int readFramesMono(double[] sampleBuffer, int numFramesToRead) throws IOException, WavFileException {
		...
		for (int f = 0; f < numFramesToRead; f++) {
			sampleBuffer[f] = 0.0;
			for (int c = 0; c < numChannels; c++) {
				sampleBuffer[f] += floatOffset + readSample() / floatScale;
			}
		}
