 ...
 * <p>
 * http://developer.creative.com <br>
...
public class SF2Soundbank implements Soundbank {
	...
	private void readSdtaChunk(RIFFReader riff) throws IOException {
		while (riff.hasNextChunk()) {
			...
			if (chunk.getFormat().equals("smpl")) {
				if (!largeFormat) {
					short[] sampleData = new short[chunk.available() / 2];
					byte[] buff = new byte[1024];
					int avail = chunk.available();
					int o = 0;
					while (avail != 0) {
						int readlen = (avail >= 1024) ? 1024 : avail;
						chunk.readFully(buff, 0, readlen);
						for (int i = 0; i < readlen; i += 2) {
							sampleData[o++] = ((short) ((buff[i] & 0xFF) | (buff[i + 1] << 8)));
						}
						avail -= readlen;
					}
					this.sampleData = new ModelByteBuffer(new ModelByteBuffer.ShortFormat(true, false), sampleData);
				} else {
					this.sampleData = new ModelByteBuffer(new ModelByteBuffer.ShortFormat(true, false), sampleFile, chunk.getFilePointer(), chunk.available());
				}
			}
		...
		while (riff.hasNextChunk()) {
			...
			if (format.equals("phdr")) {
				...
				for (int i = 0; i < count; i++) {
					...
					/*
					 * sample.data = new ModelByteBuffer(sampleData, (int)(start*2), (int)((end - start)*2)); if (sampleData24 != null) sample.data24 = new ModelByteBuffer(sampleData24, (int)start, (int)(end - start));
					 */
	...
	private void writeSdtaChunk(RIFFWriter writer) throws IOException {
		...
		for (SF2Sample sample : samples) {
			...
			/*
			 * smpl_chunk.write(data.array(), data.arrayOffset(), data.capacity());
			 */
		...
		for (SF2Sample sample : samples) {
			...
			/*
			 * sm24_chunk.write(data.array(), data.arrayOffset(), data.capacity());
			 */
			smpl_chunk.write(pad);
		}
	}
