...
import jp.kshoji.javax.sound.sampled.spi.AudioFileReader;
import jp.kshoji.javax.sound.sampled.spi.AudioFileWriter;
 ...
 *
 * <p>
 * Properties can be used to specify the default mixer for specific line types. Both system properties and a properties file are considered. The <code>sound.properties</code> properties file is read from an implementation-specific location
 * (typically it is the <code>lib</code> directory in the Java installation directory). If a property exists both as a system property and in the properties file, the system property takes precedence. If none is specified, a suitable
 * default is chosen among the available devices. The syntax of the properties file is specified in {@link java.util.Properties#load(InputStream) Properties.load}. The following table lists the available property keys and which methods
 * consider them:
 *
 * <table border=0>
 ...
 * </table>
 *
 * The property value consists of the provider class name and the mixer name, separated by the hash mark (&quot;#&quot;). The provider class name is the fully-qualified name of a concrete {@link jp.kshoji.javax.sound.sampled.spi.MixerProvider mixer
 * provider} class. The mixer name is matched against the <code>String</code> returned by the <code>getName</code> method of <code>Mixer.Info</code>. Either the class name, or the mixer name may be omitted. If only the class name is
 * specified, the trailing hash mark is optional.
 *
 * <p>
 * If the provider class is specified, and it can be successfully retrieved from the installed providers, the list of <code>Mixer.Info</code> objects is retrieved from the provider. Otherwise, or when these mixers do not provide a
 * subsequent match, the list is retrieved from {@link #getMixerInfo} to contain all available <code>Mixer.Info</code> objects.
 *
 * <p>
 * If a mixer name is specified, the resulting list of <code>Mixer.Info</code> objects is searched: the first one with a matching name, and whose <code>Mixer</code> provides the respective line interface, will be returned. If no matching
 * <code>Mixer.Info</code> object is found, or the mixer name is not specified, the first mixer from the resulting list, which provides the respective line interface, will be returned.
 *
 * For example, the property <code>javax.sound.sampled.Clip</code> with a value <code>&quot;com.sun.media.sound.MixerProvider#SunClip&quot;</code> will have the following consequences when <code>getLine</code> is called requesting a
 * <code>Clip</code> instance: if the class <code>com.sun.media.sound.MixerProvider</code> exists in the list of installed mixer providers, the first <code>Clip</code> from the first mixer with name <code>&quot;SunClip&quot;</code> will be
 * returned. If it cannot be found, the first <code>Clip</code> from the first mixer of the specified provider will be returned, regardless of name. If there is none, the first <code>Clip</code> from the first <code>Mixer</code> with name
 * <code>&quot;SunClip&quot;</code> in the list of all mixers (as returned by <code>getMixerInfo</code>) will be returned, or, if not found, the first <code>Clip</code> of the first <code>Mixer</code>that can be found in the list of all
 * mixers is returned. If that fails, too, an <code>IllegalArgumentException</code> is thrown.
 *
 * @author Kara Kytle
...
public class AudioSystem {
	...
	/**
	 * An integer that stands for an unknown numeric value. This value is appropriate only for signed quantities that do not normally take negative values. Examples include file sizes, frame sizes, buffer sizes, and sample rates. A number
	 * of Java Sound constructors accept a value of <code>NOT_SPECIFIED</code> for such parameters. Other methods may also accept or return this value, as documented.
	 */
	 ...
	 * 
	 * @param info
	 *            a <code>Mixer.Info</code> object representing the desired mixer, or <code>null</code> for the system default mixer
	 ...
	 * 
	 * @param info
	 *            a <code>Line.Info</code> object that specifies the kind of lines about which information is requested
	 * @return an array of <code>Line.Info</code> objects describing source lines matching the type requested. If no matching source lines are supported, an array of length 0 is returned.
	 *
	 ...
	 * @param info
	 *            a <code>Line.Info</code> object that specifies the kind of lines about which information is requested
	 * @return an array of <code>Line.Info</code> objects describing target lines matching the type requested. If no matching target lines are supported, an array of length 0 is returned.
	 *
	...
	/**
	 * Indicates whether the system supports any lines that match the specified <code>Line.Info</code> object. A line is supported if any installed mixer supports it.
	 * 
	 * @param info
	 *            a <code>Line.Info</code> object describing the line for which support is queried
	 * @return <code>true</code> if at least one matching line is supported, otherwise <code>false</code>
	 *
	...
	/**
	 * Obtains a line that matches the description in the specified <code>Line.Info</code> object.
	 *
	 * <p>
	 * If a <code>DataLine</code> is requested, and <code>info</code> is an instance of <code>DataLine.Info</code> specifying at least one fully qualified audio format, the last one will be used as the default format of the returned
	 * <code>DataLine</code>.
	 *
	 * <p>
	 * If system properties <code>javax.sound.sampled.Clip</code>, <code>javax.sound.sampled.Port</code>, <code>javax.sound.sampled.SourceDataLine</code> and <code>javax.sound.sampled.TargetDataLine</code> are defined or they are defined in
	 * the file &quot;sound.properties&quot;, they are used to retrieve default lines. For details, refer to the {@link AudioSystem class description}.
	 *
	 * If the respective property is not set, or the mixer requested in the property is not installed or does not provide the requested line, all installed mixers are queried for the requested line type. A Line will be returned from the
	 * first mixer providing the requested line type.
	 *
	 * @param info
	 *            a <code>Line.Info</code> object describing the desired kind of line
	 ...
	 * @throws IllegalArgumentException
	 *             if the system does not support at least one line matching the specified <code>Line.Info</code> object through any installed mixer
	 */
	...
	/**
	 * Obtains a clip that can be used for playing back an audio file or an audio stream. The returned clip will be provided by the default system mixer, or, if not possible, by any other mixer installed in the system that supports a
	 * <code>Clip</code> object.
	 *
	 * <p>
	 * The returned clip must be opened with the <code>open(AudioFormat)</code> or <code>open(AudioInputStream)</code> method.
	 *
	 * <p>
	 * This is a high-level method that uses <code>getMixer</code> and <code>getLine</code> internally.
	 *
	 * <p>
	 * If the system property <code>javax.sound.sampled.Clip</code> is defined or it is defined in the file &quot;sound.properties&quot;, it is used to retrieve the default clip. For details, refer to the {@link AudioSystem class
	 * description}.
	 *
	 * @return the desired clip object
	 *
	 * @throws LineUnavailableException
	 *             if a clip object is not available due to resource restrictions
	 * @throws SecurityException
	 *             if a clip object is not available due to security restrictions
	 * @throws IllegalArgumentException
	 *             if the system does not support at least one clip instance through any installed mixer
	 *
	 ...
	 *
	 * @throws LineUnavailableException
	 *             if a clip is not available from this mixer due to resource restrictions
	 * @throws SecurityException
	 *             if a clip is not available from this mixer due to security restrictions
	 * @throws IllegalArgumentException
	 *             if the system does not support at least one clip through the specified mixer
	 *
	 * @see #getClip()
	 * @since 1.5
	 */
	...
	/**
	 * Obtains a source data line that can be used for playing back audio data in the format specified by the <code>AudioFormat</code> object. The returned line will be provided by the default system mixer, or, if not possible, by any other
	 * mixer installed in the system that supports a matching <code>SourceDataLine</code> object.
	 *
	 * <p>
	 * The returned line should be opened with the <code>open(AudioFormat)</code> or <code>open(AudioFormat, int)</code> method.
	 *
	 * <p>
	 * This is a high-level method that uses <code>getMixer</code> and <code>getLine</code> internally.
	 *
	 * <p>
	 * The returned <code>SourceDataLine</code>'s default audio format will be initialized with <code>format</code>.
	 *
	 * <p>
	 * If the system property <code>javax.sound.sampled.SourceDataLine</code> is defined or it is defined in the file &quot;sound.properties&quot;, it is used to retrieve the default source data line. For details, refer to the
	 * {@link AudioSystem class description}.
	 *
	 * @param format
	 *            an <code>AudioFormat</code> object specifying the supported audio format of the returned line, or <code>null</code> for any audio format
	 * @return the desired <code>SourceDataLine</code> object
	 *
	 * @throws LineUnavailableException
	 *             if a matching source data line is not available due to resource restrictions
	 * @throws SecurityException
	 *             if a matching source data line is not available due to security restrictions
	 * @throws IllegalArgumentException
	...
	/**
	 * Obtains a source data line that can be used for playing back audio data in the format specified by the <code>AudioFormat</code> object, provided by the mixer specified by the <code>Mixer.Info</code> object.
	 *
	 * <p>
	 * The returned line should be opened with the <code>open(AudioFormat)</code> or <code>open(AudioFormat, int)</code> method.
	 *
	 ...
	 *
	 * @throws LineUnavailableException
	 *             if a matching source data line is not available from the specified mixer due to resource restrictions
	 * @throws SecurityException
	 *             if a matching source data line is not available from the specified mixer due to security restrictions
	 * @throws IllegalArgumentException
	 *             if the specified mixer does not support at least one source data line supporting the specified audio format
	 *
	...
	/**
	 * Obtains a target data line that can be used for recording audio data in the format specified by the <code>AudioFormat</code> object. The returned line will be provided by the default system mixer, or, if not possible, by any other
	 * mixer installed in the system that supports a matching <code>TargetDataLine</code> object.
	 *
	 * <p>
	 * The returned line should be opened with the <code>open(AudioFormat)</code> or <code>open(AudioFormat, int)</code> method.
	 *
	 ...
	 *
	 * <p>
	 * The returned <code>TargetDataLine</code>'s default audio format will be initialized with <code>format</code>.
	 *
	 * <p>
	 * If the system property {@code javax.sound.sampled.TargetDataLine} is defined or it is defined in the file &quot;sound.properties&quot;, it is used to retrieve the default target data line. For details, refer to the {@link AudioSystem
	 * class description}.
	 *
	 * @param format
	 *            an <code>AudioFormat</code> object specifying the supported audio format of the returned line, or <code>null</code> for any audio format
	 ...
	 *
	 * @throws LineUnavailableException
	 *             if a matching target data line is not available due to resource restrictions
	 * @throws SecurityException
	 *             if a matching target data line is not available due to security restrictions
	 * @throws IllegalArgumentException
	 *             if the system does not support at least one target data line supporting the specified audio format through any installed mixer
	 *
	...
	/**
	 * Obtains a target data line that can be used for recording audio data in the format specified by the <code>AudioFormat</code> object, provided by the mixer specified by the <code>Mixer.Info</code> object.
	 *
	 * <p>
	 * The returned line should be opened with the <code>open(AudioFormat)</code> or <code>open(AudioFormat, int)</code> method.
	 *
	 ...
	 *
	 * @throws LineUnavailableException
	 *             if a matching target data line is not available from the specified mixer due to resource restrictions
	 * @throws SecurityException
	 *             if a matching target data line is not available from the specified mixer due to security restrictions
	 * @throws IllegalArgumentException
	 *             if the specified mixer does not support at least one target data line supporting the specified audio format
	 *
	 ...
	 * @return array of encodings. If <code>sourceEncoding</code>is not supported, an array of length 0 is returned. Otherwise, the array will have a length of at least 1, representing <code>sourceEncoding</code> (no conversion).
	 */
	 ...
	 * @return array of encodings. If <code>sourceFormat</code>is not supported, an array of length 0 is returned. Otherwise, the array will have a length of at least 1, representing the encoding of <code>sourceFormat</code> (no
	 *         conversion).
	 */
	...
	/**
	 * Obtains the audio file format of the provided input stream. The stream must point to valid audio file data. The implementation of this method may require multiple parsers to examine the stream to determine whether they support it.
	 * These parsers must be able to mark the stream, read enough data to determine whether they support the stream, and, if not, reset the stream's read pointer to its original position. If the input stream does not support these
	 * operations, this method may fail with an <code>IOException</code>.
	 * 
	 * @param stream
	...
	/**
	 * Obtains an audio input stream from the provided input stream. The stream must point to valid audio file data. The implementation of this method may require multiple parsers to examine the stream to determine whether they support it.
	 * These parsers must be able to mark the stream, read enough data to determine whether they support the stream, and, if not, reset the stream's read pointer to its original position. If the input stream does not support these
	 * operation, this method may fail with an <code>IOException</code>.
	 * 
	 * @param stream
	 ...
	 * @return <code>true</code> if the file type is supported for this audio input stream, otherwise <code>false</code>
	 */
	...
	/**
	 * Writes a stream of bytes representing an audio file of the specified file type to the output stream provided. Some file types require that the length be written into the file header; such files cannot be written from start to finish
	 * unless the length is known in advance. An attempt to write a file of such a type will fail with an IOException if the length in the audio file type is <code>AudioSystem.NOT_SPECIFIED</code>.
	 *
	 * @param stream
	 *            the audio input stream containing audio data to be written to the file
	 * @param fileType
	 ...
	 * 
	 * @return an array of {@link jp.kshoji.javax.sound.sampled.spi.FormatConversionProvider FormatConversionProvider} objects representing the available format converters. If no format converters readers are available on the system, an array of
	 ...
	 * 
	 * @return a List of {@link jp.kshoji.javax.sound.sampled.spi.AudioFileReader AudioFileReader} objects representing the installed audio file readers. If no audio file readers are available on the system, an empty List is returned.
	 */
	 ...
	 * 
	 * @return a List of {@link javax.sound.samples.spi.AudioFileWriter AudioFileWriter} objects representing the available audio file writers. If no audio file writers are available on the system, an empty List is returned.
	 */
	 ...
	 * @param info
	 *            The requested line type TargetDataLine.class, Clip.class or Port.class.
	...
	private static Mixer getDefaultMixer(List providers, Line.Info info) {
		...
		String providerClassName = "DefaultProvider"; // bjo JDK13Services.getDefaultProviderClassName(lineClass);
		String instanceName = "DefaultInstance"; // bjo JDK13Services.getDefaultInstanceName(lineClass);
		Mixer mixer;
		...
		/*
		 * Provider class not specified or provider class cannot be found, or provider class and instance specified and instance cannot be found or is not appropriate
		 */
		if (instanceName != null) {
		...
		/*
		 * No default are specified, or if something is specified, everything failed.
		 */
		return null;
	}
	 ...
	 *            If true, only Mixers that support mixing are returned for line types of SourceDataLine and Clip.
	 * 
	...
	/**
	 * Checks if a Mixer is appropriate. A Mixer is considered appropriate if it support the given line type. If isMixingRequired is true and the line type is an output one (SourceDataLine, Clip), the mixer is appropriate if it supports at
	 * least 2 (concurrent) lines of the given type.
	 * 
	 * @return true if the mixer is considered appropriate according to the rules given above, false otherwise.
	 */
	 ...
	 * 
	 * @return a List of instances of providers for the requested service. If no providers are available, a vector of length 0 will be returned.
	 */
	private static List getProviders(Class providerClass) {
		return new ArrayList();
//bjo		return JDK13Services.getProviders(providerClass);
	}
}
