...
/**
 * MIDI Input Device stop() method must be called when the application will be destroyed.
 * 
 * @author K.Shoji
 */
public final class MidiInputDevice {
	...
	public MidiInputDevice(UsbDevice usbDevice, UsbDeviceConnection usbDeviceConnection, UsbInterface usbInterface, UsbEndpoint usbEndpoint, OnMidiInputEventListener midiEventListener) throws IllegalArgumentException {
		...
		System.out.println("MidiInputDevice: " // 
				+ "vendor " +usbDevice.getVendorId() //
	...
	/**
	 * Polling thread for input data. Loops infinitely while stopFlag == false.
	 * 
	 * @author K.Shoji
	 */
	final class WaiterThread extends Thread {
		 ...
		 */
		WaiterThread() {
			super("MidiInputDevice.WaiterThread");
			stopFlag = false;
		}
		...
		public void run() {
			...
			// bjo -- use max packet size for buffer length (was hard-coded to 64 before)
			int BUFFER_LENGTH = inputEndpoint.getMaxPacketSize();

			System.out.println("MidiInputDevice.WaiterThread is running, using buffer length " + BUFFER_LENGTH);
			...
			inputEndpoint.getMaxPacketSize();
			...
			while (!stopFlag) {
				...
				if (length < 0) {
//					stopFlag = true;
//					System.out.println("MidiInputDevice: Error " + length + " received, pausing");
					try {
						Thread.sleep(1000);
					} catch (Exception e) {
					}
				...
				if (length > 0) {
					...
					for (int i = 0; i < readSize; i += 4) {
						...
						//System.out.println("RAW MIDI: "+codeIndexNumber + " "+byte1+" "+byte2+" "+byte3);
