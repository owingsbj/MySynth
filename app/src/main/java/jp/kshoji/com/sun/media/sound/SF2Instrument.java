		...
		for (SF2InstrumentRegion presetzone : regions) {
			...
			for (SF2LayerRegion layerzone : layer.getRegions()) {
				...
				if (startAddrsOffset != 0 || endAddrsOffset != 0) {
					...
					/*
					 * if (startAddrsOffset < 0) startAddrsOffset = 0; if (endAddrsOffset > (buff.capacity()/2-startAddrsOffset)) startAddrsOffset = (int)buff.capacity()/2-startAddrsOffset; byte[] data = buff.array(); int off =
					 * (int)buff.arrayOffset() + startAddrsOffset*2; int len = (int)buff.capacity() + endAddrsOffset*2; if (off+len > data.length) len = data.length - off; buff = new ModelByteBuffer(data, off, len); if(buff24 != null) {
					 * data = buff.array(); off = (int)buff.arrayOffset() + startAddrsOffset; len = (int)buff.capacity() + endAddrsOffset; buff24 = new ModelByteBuffer(data, off, len); }
					 */
				}
				...
				performer.getConnectionBlocks()
						.add(new ModelConnectionBlock(new ModelSource(ModelSource.SOURCE_LFO2, ModelStandardTransform.DIRECTION_MIN2MAX, ModelStandardTransform.POLARITY_BIPOLAR, ModelStandardTransform.TRANSFORM_LINEAR),
								new ModelSource(new ModelIdentifier("midi_cc", "1", 0), ModelStandardTransform.DIRECTION_MIN2MAX, ModelStandardTransform.POLARITY_UNIPOLAR, ModelStandardTransform.TRANSFORM_LINEAR), 50,
